...
# List of airline company names for mentions
airline_names = [
...
def is_complaint(text: str) -> bool:
    """True if text matches any complaint pattern (keep these)."""
    return any(r.search(text) for r in complaint_regex)


def is_promotional_spam(text: str) -> bool:
    """True if text matches promotional spam (drop if also short URL)."""
...
def contains_airline_tag_or_is_quote_or_reply(tweet: Dict) -> bool:
    """Check if the tweet tags an airline, is a quote, or is a reply."""
    user_mentions = tweet.get('entities', {}).get('user_mentions', [])
    tagged_airlines = {m.get('screen_name', '').lower() for m in user_mentions}
    if any(a.lower() in tagged_airlines for a in airline_names):
        return True
...
def plot_summary_stats(stats: Dict[str, int]):
    """Plot and save a bar chart of processing statistics."""
...
def main():
        ...
        with open(json_file, 'r', encoding='utf-8') as infile:
            for line in infile:
                ...
                if contains_airline_tag_or_is_quote_or_reply(tweet):
                    ...
                    # Get full text for spam check
                    text = tweet.get('text') or tweet.get('extended_tweet', {}).get('full_text', '')
                    # Drop promotional spam unless it's a genuine complaint
    ...
    # After all files are done, plot stats
