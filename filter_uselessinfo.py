...
# Havayolu şirket isimleri listesi
airline_names = [
...
def contains_airline_name(text):
    """Tweet içinde havayolu ismi geçiyor mu kontrol et"""
    if text is None:
        return False
    text = text.lower()
...
for filename in tqdm(os.listdir(json_dir_path), desc="Dosyalar işleniyor"):
    if filename.endswith('.json'):
        ...
        # Her dosya için satır sayısını hesaplayalım
        ...
        with open(json_file_path, 'r', encoding='utf-8') as file, open(output_file_path, 'w', encoding='utf-8') as output_file:
            for line in tqdm(file, total=total_lines, desc=f"{filename} işleniyor", leave=False):
                try:
